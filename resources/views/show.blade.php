                        ...
                        @elseif($post->userKeyValue == Auth::user()->email)
                            <div class="flex justify-end mt-10">
                                <a href="{{ route('posts.edit', $post->id) }}"
                                    class="bg-gray-200 w-10 h-6 leading-6 text-center hover:bg-gray-300 mr-3">수정</a>
                                <form action="{{ route('posts.destroy', $post->id) }}" method="post"
                                    onsubmit="return confirm('글을 삭제하겠습니까?')">
                                    @method('delete')
                                    @csrf
                                    <input
                                        class="bg-gray-200 w-10 h-6 leading-6 text-center hover:bg-gray-300 text-center hover:bg-gray-300"
                                        type="submit" value="삭제" class="hover:text-blue-600 cursor-pointer">
                            </div>
                        @endif
                    </div>
                </div>
